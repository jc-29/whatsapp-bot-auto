...
class TriggerManager {
  ...
  /**
   * Handle WhatsApp incoming message with interactive multi-step workflow.
    ...
    try {
      ...
      if (body.toUpperCase() === 'CANCEL' || body.toUpperCase() === '#CANCEL') {
        if (this.userSessions.has(senderNumber)) {
          ...
          await client.sendMessage(targetChatJid, '❌ *Interactive campaign setup canceled.* Send #COMPOSEMESSAGE whenever you want to start a new campaign.');
        }
        return;
      }

      // 1. Step 0: User sends #COMPOSEMESSAGE
      if (this.matchesComposeCodeword(body)) {
        ...
        await client.sendMessage(
          targetChatJid,
          `📝 *Interactive Campaign Setup Started!*\n\nPlease reply with the **Message / Template** you want to broadcast.\n• You can attach an **Image** or **Video** with your text as a caption!\n• You can use dynamic tags like \`{Name}\`, \`{Status}\`, \`{Date}\`, etc.\n\n*(Reply CANCEL anytime to exit setup)*`
        );
        return;
      }
      ...
      // 2. Step 1: User sends message text or attached image/video
      if (session && session.step === 'AWAITING_MESSAGE') {
        ...
        await client.sendMessage(
          targetChatJid,
          `✅ *Message Template Recorded!*\n${mediaItem ? '📷 Media Attachment Received.\n' : ''}Preview: "${templateText}"\n\nNext, please reply with your **Google Spreadsheet URL**\n*(e.g., https://docs.google.com/spreadsheets/d/1BxiMVs...)*`
        );
        return;
      }

      // 3. Step 2: User sends Google Spreadsheet Link
      if (session && session.step === 'AWAITING_SPREADSHEET') {
        if (!body.includes('docs.google.com/spreadsheets') && !body.includes('/d/')) {
          await client.sendMessage(targetChatJid, `⚠️ Please provide a valid Google Spreadsheet link (e.g., \`https://docs.google.com/spreadsheets/d/...\`) or reply CANCEL.`);
          return;
        }
        ...
        await client.sendMessage(
          targetChatJid,
          `📊 *Google Spreadsheet URL Recorded!*\n\nNext, reply with the **Phone Number Column Name**\n*(e.g., Phone, Mobile, WhatsApp — or reply AUTO to automatically detect it)*`
        );
        return;
      }

      // 4. Step 3: User sends Phone Number Column Name
      if (session && session.step === 'AWAITING_COLUMN') {
        ...
        await client.sendMessage(
          targetChatJid,
          `📋 *Campaign Setup Complete & Ready for Broadcast!*\n\n• **Spreadsheet**: ${session.sheetUrl}\n• **Phone Column**: ${session.phoneColumn || 'Auto-Detected'}\n• **Media Attached**: ${session.mediaItem ? 'Yes' : 'No'}\n• **Message Template**:\n"${session.template}"\n\nTo confirm and start sending WhatsApp messages to all numbers in the spreadsheet, reply with:\n🚀 *#STARTBROADCAST*\n\n*(Or reply CANCEL to discard this setup)*`
        );
        return;
      }
