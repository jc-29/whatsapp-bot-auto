      ...
      if (session && session.step === 'AWAITING_MESSAGE') {
        ...
        if (hasMedia) {
          try {
            ...
            // Initial 2-second pause to allow WhatsApp Web tab to complete CDN media key decryption
            await new Promise(r => setTimeout(r, 2000));
            
            let media = null;

            // Retry downloading media up to 5 times (1.5s interval)
            for (let attempt = 1; attempt <= 5; attempt++) {
              try {
                media = await msg.downloadMedia();
                if (media && media.data) break;
              } catch (e) {
                this.log(`Media download attempt ${attempt} failed: ${e.message}`, 'warning');
              }

              // Fallback: try refetching chat message if attempt >= 2
              if (!media || !media.data) {
                try {
                  const chat = await msg.getChat();
                  const messages = await chat.fetchMessages({ limit: 3 });
                  const targetMsg = messages.find(m => m.id._serialized === msg.id._serialized);
            ...
            if (media && media.data) {
              ...
              throw new Error('WhatsApp Web media blob was not ready after retries');
            }
          } catch (mediaErr) {
