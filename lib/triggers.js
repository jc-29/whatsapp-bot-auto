...
class TriggerManager {
  ...
  async handleWhatsAppMessage(msg, client) {
    try {
      ...
      this.log(`🚀 Codeword '${config.codeword}' triggered via WhatsApp by ${senderNumber}`, 'success');
      await client.sendMessage(targetChatJid, `🤖 *Codeword Received!* Starting Google Spreadsheet data extraction & WhatsApp broadcast pipeline...`);

      // Execute trigger callback
      const result = await this.executeTrigger({ source: 'WhatsApp', sender: senderNumber });
      
      await client.sendMessage(targetChatJid, `✅ *Broadcast Completed!*\n\n- Sent: ${result.success}\n- Failed: ${result.failed}\n- Total Processed: ${result.total}`);
    } catch (err) {
